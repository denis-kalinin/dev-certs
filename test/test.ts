...
describe("office-addin-dev-certs", function() {
    ...
    describe("generate-tests", function() {
        ...
        it("certificate not installed, ensureDir fails case", async function() {
            ...
            try {
                await generate.generateCertificates(testCaCertificatePath, testCertificatePath, testKeyPath, pkiConfig);
                // expecting exception
    ...
    describe("install-tests", function() {
        ...
        if (process.platform === "win32") {
            ...
            it("with --machine option", async function() {
                ...
                assert.strictEqual(execSync.callCount, 1);
                assert.strictEqual(execSync.calledWith(`powershell -ExecutionPolicy Bypass -File "${script}" ${machine ? "LocalMachine" : "CurrentUser"} "${testCaCertificatePath}"`), true);
            });
